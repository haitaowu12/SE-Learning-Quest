...
function focusQuestContent(): void {
  requestAnimationFrame(() => document.querySelector<HTMLElement>('.rpg [data-quest-focus]')?.focus());
}
    ...
    {!save.onboarded && view !== 'settings' ? <Onboarding save={save} onBegin={(name, origin) => {
      ...
      <main id="rpg-main" className="rpg-main" tabIndex={-1}>
        ...
        {view === 'quest' && current && <QuestScene key={current.id} quest={current} save={save} state={state} feedback={feedback} onChoose={(decisionId, choice) => {
          try { update(choose(save, decisionId, choice.id)); setFeedback(choice); focusQuestContent(); } catch (error) { setNotice(String(error)); }
        }} onContinue={() => { setFeedback(null); focusQuestContent(); }} onSubmit={(answer, assisted) => act(() => submitPuzzle(save, answer, assisted))} onClaim={claim} onRetry={() => {
          act(() => retryQuest(save)); setFeedback(null); focusQuestContent();
  ...
  return <>
    ...
    <div className="rpg-quest-layout"><div>
      ...
      {!record?.completed && !feedback && decision && <section className="rpg-dialogue" data-decision={decision.id}>
        ...
        <h2 data-quest-focus tabIndex={-1}>{decision.prompt}</h2><p>{decision.context}</p><div className="rpg-choices">{decision.options.map((choice, i) => <button className="rpg-choice" key={choice.id} data-choice={choice.id} onClick={() => onChoose(decision.id, choice)}><span aria-hidden="true">{i + 1}</span><strong>{choice.label}</strong><i aria-hidden="true">↗</i></button>)}</div>
      </section>}
...
function PuzzleBench({ puzzle, record, onSubmit }: { puzzle: Puzzle; record: Save['records'][string]; onSubmit: (answer: string[] | Record<string, string>, assisted?: boolean) => void }) {
  ...
  return <section className="rpg-bench" data-puzzle={puzzle.id}><span className="rpg-kicker">AT THE WORKBENCH · {puzzle.kind === 'trade' ? 'COMPARE DESIGNS' : puzzle.kind === 'match' ? 'CONNECT THE EVIDENCE' : puzzle.kind === 'order' ? 'PLAN THE SEQUENCE' : 'BUILD AN EVIDENCE SET'}</span><h2 data-quest-focus tabIndex={-1}>{puzzle.title}</h2><p>{puzzle.prompt}</p>
