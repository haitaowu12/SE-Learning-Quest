...
| --- | --- |
| Clean strong campaign | All 24 quests through the rendered UI, strong-choice feedback, every required bench, quest rewards, level progression, training, journal access, reload persistence, all 48 field-guide entries, final badge and strong ending. The route blocks external network requests and fails on page errors, console errors or HTTP error responses. |
...
| Responsive keyboard and save controls | 390 px and 320 px viewports, skip-link keyboard order/activation, keyboard choice activation, reconsideration, 200% **application text size**, contrast and reduced-motion settings, axe scans, no horizontal overflow, export, rejected malformed import, reset confirmation/Escape, valid import and the legacy-episode return path. |
...
- `npm test`: **76/76 passed**, including 24 engine and 31 persistence tests plus legacy/content coverage.
...
The local production preview verifies static assets and repository-subpath behavior. It does not establish that a new commit has been deployed to the public GitHub Pages URL. Remote publication remains proven only by a successful repository workflow and a check of the resulting deployed URL.
