# Coffee Lab and Rail Quest

These two earlier episodes remain available through **Other episodes** in Asterfall, or the `#episodes` URL fragment. Asterfall is now the default experience. See the root README for current development, testing and deployment instructions.
...
The Vite base path is `./`, so assets resolve under the repository path, a renamed fork, or a custom-domain root.
